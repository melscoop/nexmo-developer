  ...
  def directory_hash(path, name = nil)
    ...
    if DocumentationConstraint.product_with_parent_list.include? product
      ...
      if tasks['tutorials']&.size&.positive?
        children = children.concat(tasks['tutorials'])
        if tasks['use_cases']&.size&.positive?
          children << { title: 'use-cases', path: "/#{product}/tutorials", children: tasks['use_cases'] }
        end
        data[:children] << { title: 'tutorials', path: "/#{product}/tasks", children: children }
      elsif tasks['use_cases']&.size&.positive?
        # Otherwise show tutorials as the top level
        data[:children] << { title: 'tutorials', path: "/#{product}/tutorials", children: tasks['use_cases'] }
      end
    end
